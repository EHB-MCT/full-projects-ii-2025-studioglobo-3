document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateFilters() {
    kaarten.forEach((kaart) => {
      const matchLevel = selectedLevel
        ? kaart.classList.contains(selectedLevel)
        : true;
      const matchType = selectedType
        ? kaart.classList.contains(selectedType)
        : true;
    ...
    function cfUpdate() {
      cfCircles.forEach((circle) => {
        circle.classList.toggle(
          "active",
          Number(circle.dataset.step) === cfCurrent
        );
      });
      cfSteps.forEach((step) => {
        step.classList.toggle(
          "active",
          Number(step.dataset.step) === cfCurrent
        );
      });
  ...
  if (readModal1 && openReadBtn1) {
    ...
    openReadBtn1.addEventListener(
      "click",
      () => (readModal1.style.display = "block")
    );
    close1.addEventListener("click", () => (readModal1.style.display = "none"));
    window.addEventListener("click", (e) => {
      if (e.target === readModal1) readModal1.style.display = "none";
    });
  }
